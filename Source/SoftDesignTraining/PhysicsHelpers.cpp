...
bool PhysicsHelpers::CastRay( const FVector& start,const FVector& end, TArray<struct FHitResult>& outHits, bool drawDebug, RayCastChannel channel)
{
    if( m_world == nullptr )
        return false;
    ...
    if (channel == RayCastChannel::default)
    {
...
bool PhysicsHelpers::SphereCast(const FVector& start,const FVector& end, float radius, TArray<struct FHitResult>& outHits, bool drawDebug, RayCastChannel channel)
{
    if( m_world == nullptr )
        return false;
    ...
    if (channel == RayCastChannel::deathfloor)
    {
        objectQueryParams.AddObjectTypesToQuery(ECollisionChannel::ECC_GameTraceChannel3);
    }
...
bool PhysicsHelpers::SphereOverlap( const FVector& pos, float radius, TArray<struct FOverlapResult>& outOverlaps, bool drawDebug, RayCastChannel channel)
{
    if( m_world == nullptr )
        return false;
